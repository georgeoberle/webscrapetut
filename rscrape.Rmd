...
This is a tutorial meant for historians to use in order to use the rvest package to scrape the web. It was created as one of several requirements for a course taught by Lincoln Mullen at George Mason University.
...
You will see that the data is not perfect but it has been Now that this data is saved into a csv file it is can easily cleaned and made into tidy data using Hadley Wickham's packages dplyr and tidyr.
...
carnlib %>%
 ...
 #Problems with this is that there are odd character transfers that are not numeric.
```
